$(function () {
        ...
        $("#jqGrid1").jqGrid({
            ...
            colModel: [
                ...
                // },
                {
                    ...
                    align: 'center',
                    edittype: 'select',
                    editoptions: {value: '20:售后检测(待接收)'},
                    formatter: function (value) {                   
                        switch(value) {
                            case '20': return '售后检测(待接收)'
                                break;
   ...
   function fn2() {
        ...
        $("#jqGrid2").jqGrid({
            ...
            mtype: 'post',
            rownumbers: true,
            postData: {pageCode: 'shjc'}, // 查询未检测 and 装机中
            multiselect : true, // 允许多选
            colModel: [
                {
                    label: '时间',
                ...
                },
                {
                    ...
                    editable: true,                   
                    edittype: 'select',
                    editoptions: {value: '1:退货;2:换货;3:维修'},
                    formatter: function (value) {                   
                        switch(value) {
                            case '1': return '退货'
                                break;  
            ...
            // options for the Edit Dialog
            {
               ...
               // checkOnSubmit : true,
                ...
                afterShowForm : function (formid) { // 表单显示后触发      
                    ...
                    $('#tr_IMEI').hide()
                    $('#testStatus').attr('disabled','true')
                    $('#shoption').attr('disabled','true')
                    // $('#remark').val('')
                },
                beforeCheckValues:function(posdata,formid,mode){ // 表单验证数据之前
                    if (posdata.shoption == 1) {posdata.testStatus = '30'}
                    if (posdata.shoption == 2) {posdata.testStatus = '40'}
                    if (posdata.shoption == 3) {posdata.testStatus = '50'}              
                },
                afterSubmit:  function (response, postdata, formid) {
